...
const OtherIncomeRV = ({route, navigation}) => {
  ...
  const filterOtherIncomeData = useMemo(() => {
    if (otherIncome.length === 0) return otherIncome;
    if (searchText === '') return otherIncome;

    return otherIncome.filter(item => {
      return (
        item.title.includes(searchText) || item.description.includes(searchText)
      );
    });
  }, [searchText, otherIncome]);

  const onDelete = async id => {
    console.log('Delete ID', id);
    setLoading(true);
    axios
      .delete('/api/otherincome/?id=' + id)
      .then(res => {
  ...
  return (
    <View style={{flex: 1, padding: 10, backgroundColor: '#f0f0f0'}}>
      <View style={{flexDirection: 'row', marginBottom: 5}}>
        ...
        <Text style={{...s.bold_label}}>OtherIncome Receipt</Text>
      </View>
      ...
      <View
        ...
        <TextInput
          ...
          placeholder={'Search Otherincome'}
